...
def get_model(use_aux_loss=False, PRETRAINED=False, FREEZE=False, weight_path=None):
    if PRETRAINED:
        model = torch.hub.load('pytorch/vision:v0.6.0', 'inception_v3',
                               pretrained=True,
                               force_reload=False,
                               aux_logits=use_aux_loss)
        # model = torch.hub.load('pytorch/vision:v0.6.0', 'resnet18', num_classes=1)
        
        adjust_model = True
        if adjust_model:
            old_conv_weights = model.Conv2d_1a_3x3.conv.weight.data
            model.Conv2d_1a_3x3.conv = nn.Conv2d(3, 32, kernel_size=3, stride=1, bias=False) 
            model.Conv2d_1a_3x3.conv.weight.data = old_conv_weights

        model.fc = torch.nn.Linear(in_features=2048, out_features=1, bias=True)
        
        #model.pool2 = nn.AdaptiveAvgPool2d(1)
        if use_aux_loss:
            model.AuxLogits.fc = torch.nn.Linear(in_features=768, out_features=1, bias=True)
    else:
        model = inception_v3(aux_logits=use_aux_loss)
